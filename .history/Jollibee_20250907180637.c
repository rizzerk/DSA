...
{
	MealOrder order = {NULL, 0, {cust.custID, {cust.custName.fName,cust.custName.mi, cust.custName.lName}}, 0.0, 0};
	return order;
}
...
void addMealToOrder(MealOrder* MO, Menu menuList, Meal newOrder)
{
	...
	if(i < MENUSIZE){
		...
		if(j < MO->ordItems){ // existing order
			...
			newMeal->delQty =0;
			newMeal->link =NULL;
			newMeal->order.price = newOrder.price;

			*temp = newMeal;
			
			MO->ordItems ++;
			MO->ordStat = 0;
			MO->bill += newMeal->order.price;
		}
...
void deliverMeals(MealOrder* MO, Meal DList[], int numMeals)
{
	...
	{
		// for(i =0, trav = &MO->ML; i < MO->ordItems && )
