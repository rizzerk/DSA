...
void addMealToOrder(MealOrder* MO, Menu menuList, Meal newOrder)
{
	...
	if(i < MENUSIZE){
		int j;
		MealList *temp;
		for(temp = &MO->ML; *temp != NULL && strcmp(newOrder.mealCode, (*temp)->order.mealCode) != 0; temp = &(*temp)->link); //if neworder exist in the MealOrder
		if(*temp != NULL){ // existing order
			(*temp)->qty ++;
		...
		printf("\nWe dont have the %s in our Store.\n", newOrder.mealName);
	}
}
...
/* This function checks if individual meals in an order are complete. 
A meal is considered complete if the delivered quantity (delQty) matches the ordered quantity (qty). 
If that condition is met, the meal’s status (mealStat) is updated to 1 (delivered).
 */
void checkMealCompletion(MealOrder MO)
{
	MealList trav = MO.ML;
	while(trav != NULL){
		trav->mealStat = trav->qty == trav->delQty ? 1 : 0;
		trav = trav->link;
	}
}

/* This function checks if the entire order is complete. 
An order is considered complete if all the meals in the order have been delivered. 
If that condition is met, the order’s status (ordStat) is updated to indicate completion.
 */
void checkOrderCompletion(MealOrder* MO)
{
	int i;
	MealList trav;
	for(i =0, trav = MO->ML; trav != NULL && trav->mealStat == 1; i++, trav = trav->link){}
	MO->ordStat = i <= MO->ordItems ? 1 : 0;
...
void deliverMeals(MealOrder* MO, Meal DList[], int numMeals)
{
	int i;
	MealList trav;
	for(int x=0; x < numMeals; x++) // x = index sa DList
	{
		...
		if(i < MO->ordItems){ //naa sya sa mealorder
			if(trav->delQty < trav ->qty){ //wapa na deliver tanan
				trav->delQty ++;
			} else{
				printf("\n%s has already been delivered.\n", DList[x].mealName);
			}

		}else{
			printf("\nYou didn't order a %s\n", DList[x].mealName);
		}

	}
	checkMealCompletion(*MO);
	checkOrderCompletion(MO);
	//check meal
	//check order
}

/* This function removes a meal from a customer’s order.
 If the meal exists in the order and has not yet been delivered, 
 						the function decreases its quantity or removes it entirely (if the qty reaches 0). 
 If the meal has already been delivered, the system should not allow cancellation. 
 If the customer never ordered the meal, an error message is shown.
 */

void removeMealFromOrder(MealOrder* MO, Meal cancelMeal)
{
	MealList *trav;
	for(trav = &MO->ML; *trav != NULL && strcmp(cancelMeal.mealCode,  (*trav)->order.mealCode) != 0; trav = &(*trav)->link);
	if(*trav != NULL){
		if((*trav)->mealStat == 0){ //wapa na sya deliver
			(*trav)->qty--;
		...
		}else{ //delivered na sya
		printf("All of you %s have already been delivered. I'm sorry we cannot cancel the order", cancelMeal.mealName);
		}

	} else{
		printf("You never ordered the %s", cancelMeal.mealName);
	}
