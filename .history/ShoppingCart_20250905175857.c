...
void genReceipt(Cart* C, PromoCode promoList[], char promoCode[])
{
  ...
  float discount = i < PMSIZE ? promoList[i].discount * C->receipt.totalPrice : 0.0;
...
void returnItem(Cart* C, ProdList* PL, char prodID[], int qty)
{
  ...
  SList prev;
  for(i = 0, temp= C->sCart; i < C->numItems && strcmp(temp->item.prodID, prodID) !=0 ; i++, prev = temp, temp= temp->nextItem){} //pangita sa linked list cart
  if(i < C->numItems){ //check if naa sa cart
    if(temp->item.qty >= qty){ //check if ang qty sa node (temp nga gi traverse) kay mas daghan or gamay sa iya i minus kay nganung mo return man kuno syag 5 kabook if 1 ra iya naa sa cart amp
      ...
      for(j = 0; i < PL->lastProd && strcmp(PL->prodList[i].prodID, prodID) !=0 ; j++){}
      PL->prodList[j].qty += qty;
      if(temp->item.qty == 0){ //delete na ang node 
        //ang problema unsaon ni ron nga naa man ko sa node... eerrm wapko kasuway... HAHAHAAHA
        //suway kog prev kay wako kasabot unsaon to ang notes ni sir sa ing ana nga version 
