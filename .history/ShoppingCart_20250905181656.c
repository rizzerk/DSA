...
void genReceipt(Cart* C, PromoCode promoList[], char promoCode[])
{
  ...
  // float discount = i < PMSIZE ? promoList[i].discount * C->receipt.totalPrice : 0.0; //puyde raman ata ni dristo  C->receipt.discount?
	C->receipt.discount = i < PMSIZE ? promoList[i].discount * C->receipt.totalPrice : 0.0; //puyde raman ata ni dristo
...
{
  int i, j;
  SList *temp; //notes.c ni sir (naglibog ko Pointer to pointer to a pointer ?? *** ??)
  SList copy;
  ...
  if(i < C->numItems){ //check if naa sa cart
    ...
    if((*temp)->item.qty >= qty){ //check if ang qty sa node (temp nga gi traverse) kay mas daghan or gamay sa iya i minus kay nganung mo return man kuno syag 5 kabook if 1 ra iya naa sa cart amp
      (*temp)->item.qty -= qty; //minus lang sa cart qty
      
      for(j = 0; j < PL->lastProd && strcmp(PL->prodList[i].prodID, prodID) !=0 ; j++){}
