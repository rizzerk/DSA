...
Cart initCart(void)
{
  ...
  newCart.sCart = NULL; 
  newCart.numItems = 0;
  newCart.promoCode[0] = '\0'; //empty string
...
void displayCart(Cart* C)
{
  ...
  Cart *ptr = C;
	...
	for(int i = 0; i < C->numItems; i++, ptr = ptr->sCart->nextItem){
    float n = ptr->sCart->item.qty * ptr->sCart->item.unitPrice; 
		printf("\n%-10s%-15s%-10.2f%-10d%-15.2f",ptr->sCart->item.prodID, C->sCart->item.prodName, C->sCart->item.unitPrice, C->sCart->item.qty, n);
	}
...
{
	/*
	The function will check if the passed productID exists in the ProdList and if there is enough stocks 
					 in the ProdList to accomodate your purchase. If both of these conditions are met, it will insert
					 the item into the FIRST node of your sCart. Update the necessary fields upon insertion. 
					 Note: Machine Problems will not normally say what the necessary fields are. But you can ask me if
					 	   you give up. :)
