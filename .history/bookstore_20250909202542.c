...
void insertBookBS(bookStore* BS, bookRecord newBk)
{
  			// 3) insertBookBS() - Function will insert a new book record into the bookstore if and only if the book record still does
				...
				// 					sorted in ascending order by book ID.
   BookList *temp;
   for(temp = &BS->bList; temp != NULL && newBk.bookID < BS->bList->bkRec.bookID; temp = &(*temp)->nextBook){}
   if(temp != NULL){
      BookList newNode = (BookList)malloc(sizeof(bookNode));
      newNode->bkRec.price = newBk.price;
      newNode->bkRec.stocks = newBk.stocks;
      strcpy(newNode->bkRec.bookName, newBk.bookName);
      strcpy(newNode->bkRec.bookID, newBk.bookID);
      strcpy(newNode->bkRec.shelf, newBk.shelf);
      newNode->pages++;CS
