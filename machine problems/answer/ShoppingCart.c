...
int main(void)
{
  ...
  myCart = initCart();
  // void addToCart(Cart*, ProdList*, char[], int);
  addToCart(&myCart,&groceryStore ,prod1, 30); // dili same sa instruction pero same sa test case ni sir
	addToCart(&myCart,&groceryStore ,prod1, 10);
  addToCart(&myCart,&groceryStore ,prod1, 55); // dili same sa instruction pero same sa test case ni sir
...
void pause(void)
{
	...
	getchar(); //getch wont work (maybe because I'm using macos?) idk
	system("CLS");
}
...
{ 
  ...
  printf("\n%-10s%-15s%-10.2f%-10d", PL.prodList[i].prodID, 
...
{
  int i;
  for (i=0; i < PMSIZE && strcmp(promoCode, promoList[i].promoCode) != 0; i++){} //check sa promocode
  // float discount = i < PMSIZE ? promoList[i].discount * C->receipt.totalPrice : 0.0; //puyde raman ata ni dristo C->receipt.discount?
	C->receipt.discount = i < PMSIZE ? promoList[i].discount * C->receipt.totalPrice : 0.0; //puyde radyud diay
	C->receipt.totalPrice -= C->receipt.discount;
...
void displayCart(Cart* C)
{
  ...
  C->receipt.totalPrice = 0.0;
	for(temp= C->sCart; temp!= NULL; temp = temp->nextItem){
    float n = temp->item.unitPrice * temp->item.qty; //Line Item Price
...
void addToCart(Cart* C, ProdList* PL, char prodID[], int itemQty)
{
  ...
  if(i < PL->lastProd){
    if(PL->prodList[i].qty >= itemQty){ //wala syay function nga mo check if existing sya sa cart
...
{
  int i, j;
  SList *temp; // *trav sa notes.c ni sir (naglibog ko Pointer to pointer to a pointer ?? *** ??)
  SList copy;
  for(i = 0, temp= &C->sCart; i < C->numItems && strcmp((*temp)->item.prodID, prodID) !=0 ; i++, temp= &(*temp)->nextItem){}
  if(i < C->numItems){ //check if naa sa cart
    
    if((*temp)->item.qty >= qty){ //check if ang qty sa node (temp nga gi traverse) kay mas daghan or equal sa iya i minus kay nganung mo return man kuno syag 5 kabook if 1 ra iya naa sa cart amp
      (*temp)->item.qty -= qty; //minus lang sa cart qty
      
      for(j = 0; strcmp(PL->prodList[j].prodID, prodID) !=0 ; j++){}
      PL->prodList[j].qty += qty;

      if((*temp)->item.qty == 0){ //delete na ang node 
        //ang problema unsaon ni ron nga naa man ko sa node... eerrm... HAHAHAAHA
				C->numItems --;
        copy= *temp;
				*temp = (*temp)->nextItem;
				free(copy);
