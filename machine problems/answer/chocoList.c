...
void displayChocoList(ChocoList CL, char *listDesc) 
{
	...
	printf("%10s","Quantity");
              ...
              * below this comment.                                                *
         ***********************************************************************/  
	for(CLL trav = CL.choco; trav != NULL; trav = trav->nextChoco){
		printf("\n%-10s%-15s%10d%10.2f%10d", trav->chocoProd.chocoID, trav->chocoProd.chocoDesc.name,
											trav->chocoProd.chocoDesc.weight, trav->chocoProd.chocoPrice,trav->chocoProd.chocoQty);
	}	
...
void insertLastUnique(ChocoList *CL, Chocolate X)
{
	...
	if(*trav != NULL){
		...
		printf("Choco Record: [%s] has been sucessfully updated!\n", (*trav)->chocoProd.chocoID);
	}else if(CL->numChoco < CHOCO_SIZE){
		CLL newChoco = (CLL)malloc(sizeof(ChocoNode));
		newChoco->chocoProd = X;
		newChoco->nextChoco = *trav;
		*trav = newChoco;
		CL->numChoco++;
		printf("Choco Record: [%s] has been sucessfully inserted!\n", newChoco->chocoProd.chocoID);
	}else{
		printf("There's no more space to add [%s] to our ChocoList", X.chocoID);
	}
}
...
{	if(CL->numChoco < CHOCO_SIZE){
		...
		for(trav = &CL->choco; 
			*trav != NULL && 
			strcmp((*trav)->chocoProd.chocoDesc.name, X.chocoDesc.name) < 0;
			trav = &(*trav)->nextChoco){}
...
ChocoList populateSorted(ChocoList CL)
{
	ChocoList newCL= initChocoList();
	CLL temp;
	int i;
	for(temp = CL.choco, i=0 ; i < CL.numChoco; i++, temp = temp->nextChoco){
		insertSortedName(&newCL, temp->chocoProd);
	}
...
{
	CLL i;
	for(i = CL.choco; i != NULL; i = i->nextChoco){
		CLL min = i;
		for (CLL j = i->nextChoco; j!= NULL; j = j->nextChoco)
		{
			if(strcmp(j->chocoProd.chocoID, min->chocoProd.chocoID) < 0){
				min = j;
			}
		}
