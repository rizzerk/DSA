...
PizzaOven initOven(char ovenName[]) {
    ...
    po.rear = -1;
    po.size = 0;
    po.capacity = MAX_ORDERS;
    strcpy(po.ovenName, ovenName);
...
int isOvenFull(PizzaOven *oven) {
    // TODO: Implement this function
    return (oven->rear + 1) % MAX_ORDERS == oven->front;
}
...
void addOrder(PizzaOven *oven, PizzaOrder newOrder) {
    ...
    if(isOvenFull(oven)){
        printf("Oven is full! Please wait, [%s]\n", newOrder.pizza.customerName);
        return;
    }else {
        if(isOvenEmpty(oven)){
            oven->front = oven->rear = 0;
        }else{
            oven->rear = (oven->rear + 1) % MAX_ORDERS;
        }
        oven->orders[oven->rear] = newOrder;
        printf("[%s]'s [%s] added to oven queue\n", newOrder.pizza.customerName, newOrder.pizza.pizzaType);
        oven->size++;
    }
}
...
PizzaOrder startCooking(PizzaOven *oven) {
    ...
    if(isOvenEmpty(oven)){
        printf("No orders to cook!\n");

        PizzaOrder dummy = {0};
        dummy.orderID = -1;
        strcpy(dummy.status, "NONE");
        return dummy;  
    }else {
        PizzaOrder pizza = oven->orders[oven->front]; //puyde ra order ang name whtever
        strcpy(pizza.status, "COOKING");
        if(oven->front == oven->rear){
            oven->front = oven->rear = -1;
        }else {
            oven->front = (oven->front + 1) % MAX_ORDERS;
        }
        oven->size--;
        printf("Started cooking [%s]'s [%s]\n", pizza.pizza.customerName, pizza.pizza.pizzaType);
        return pizza;
    }
}
...
PizzaOrder checkNextOrder(PizzaOven *oven) {
    ...
    if(isOvenEmpty(oven)){
        ...
        PizzaOrder order = oven->orders[oven->front];
        return order;
    }
}
...
void cookPizza(PizzaOven *oven, RestaurantStats *stats) {
    ...
    if(isOvenEmpty(oven)){
        printf("Kitchen is empty! No orders to cook!\n");
        return;
    }else{
        PizzaOrder order = startCooking(oven);
        stats->totalOrders++;
        if(order.orderID < 0){
            printf("💥 DISASTER! %s's pizza burnt to a crisp! 😞\n", order.pizza.customerName);
            stats->cancelledOrders++;
            strcpy(order.status, "CANCELLED");
        }else if(order.priority == 1){
            printf("⭐ VIP SUCCESS! %s is delighted! Bonus tip received! 😄\n", order.pizza.customerName);
            strcpy(order.status, "READY");
            stats->completedOrders++;
            stats->happyCustomers++;
            stats->totalRevenue += order.pizza.price;
        }else{
            if(rand()%2 ==0){
                printf("🍕 SUCCESS! [%s] loves their [Pizza]! 😊\n", order.pizza.customerName);
                strcpy(order.status, "READY");
                stats->completedOrders++;
                stats->happyCustomers++; 
                stats->totalRevenue += order.pizza.price;
            }else {
                printf("💔 Oh no! %s's %s order failed! 😞\n", 
                order.pizza.customerName, order.pizza.pizzaType);
                strcpy(order.status, "CANCELLED");
                stats->cancelledOrders++;
            }
        }
...
void validateKitchen(PizzaOven *oven) {
    ...
    if(isOvenEmpty(oven)){
        printf("✅ Kitchen is clean and ready!");
        return;
    }

    int waiting = 0, cooking = 0, ready = 0, cancelled = 0;
